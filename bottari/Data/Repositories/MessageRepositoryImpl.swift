...
final class MessageRepositoryImpl: MessageRepository {
    ...
    func createMessage(threadID: String, content: String) -> AnyPublisher<CreateMessageResponse, Error> {
        ...
        guard let request = apiService.createRequest(withURL: EndPoint.messages(threadID: threadID).urlString, body: body) else { return Fail(error: URLError(.badURL)).eraseToAnyPublisher() }
